# Phase 1W — Player Management & Miscellaneous

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase W**
...
| File | Lines | Why |
|------|-------|-----|
| `player.c` | 3,802 | Player creation, deletion, management |
| `connhist.c` | ~400 | Connection history |
| `polymorph.c` | ~600 | Polymorph/shapeshift system |
| `news.c` | ~500 | News system |
| `comments.c` | ~400 | Comment system |
| `boards.c` | ~800 | Board/note system |
...
### `player.c`
1. **Character creation:** The complete new character creation flow — name validation,
   race selection, class selection, stat rolling, starting equipment.
2. **Character deletion:** How characters are permanently deleted.
3. **Player file management:** How player files are created, located, and managed
   on disk.
4. **Idle/void handling:** How idle players are moved to the void room.

### `connhist.c`
5. **Connection history:** What is logged — login times, IP addresses, character names.
   How history is stored and queried.

### `polymorph.c`
6. **Polymorph system:** How shapeshift/polymorph works. What stats change. Duration.
   Available forms.

### News, Comments & Boards
7. **News system:** Multi-category news. How posts are created and displayed.
8. **Comment system:** How comments work — attached to what entities.
9. **Board system:** How message boards work — board locations, note persistence,
...
- **Append** to `ANALYSIS.md` section `## 23. Player Management & Miscellaneous`.
- **Append** to `COMMANDS.md` any player/news/board commands found.

---

## Acceptance Criteria

- [ ] Character creation flow fully documented.
- [ ] Character deletion documented.
- [ ] Connection history system documented.
- [ ] Polymorph system documented.
- [ ] News/comment/board systems documented.
