# Phase 1P — Economy & Shops

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase P**
...
| File | Lines | Why |
|------|-------|-----|
| `shops.c` | ~1,200 | Shop system — buy, sell, list, haggle, repair shops |
| `auction.c` | ~800 | Player auction system |
| `bank.c` | ~300 | Banking system |
...
### Shops
1. **Shop data:** `shop_data` structure — what items a shop buys/sells, price
   multipliers, opening hours.
2. **Buy/sell price formulas:** Exact formulas including charisma modifier, race
   modifier, item condition.
3. **`do_buy()` / `do_sell()` / `do_list()`:** Complete transaction flow.
4. **Repair shops:** `repairshop_data` — how repair costs are calculated, what can
   be repaired.
5. **Currency system:** Three currencies (gold, silver, copper) — conversion ratios,
   how transactions handle mixed currency.

### Auction
6. **Auction mechanics:** How items are listed, bid on, and sold. Timer system.
   Minimum bid increments.

### Banking
7. **Bank operations:** Deposit, withdraw, transfer. NPC banker validation.
   Interest (if any).
...
- **Append** to `ANALYSIS.md` section `## 16. Economy & Shops`.
- **Append** to `COMMANDS.md` any economy commands found.

---

## Acceptance Criteria

- [ ] Shop buy/sell price formulas extracted exactly.
- [ ] Currency system documented with conversion ratios.
- [ ] Repair cost formula extracted.
- [ ] Auction mechanics documented.
- [ ] Banking operations documented.
