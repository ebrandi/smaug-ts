# Phase 1V — Overland & Special Systems

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase V**
...
| File | Lines | Why |
|------|-------|-----|
| `overland.c` | ~2,500 | Overland map system — coordinate-based movement |
| `dragonflight.c` | ~500 | Dragon-based fast travel |
| `special.c` | ~800 | Special mob procedures |
| `weather.c` | ~500 | Weather system |
| `calendar.c` | ~300 | In-game calendar |
...
### Overland Map
1. **Coordinate system:** How the overland map uses X/Y coordinates instead of vnums.
2. **Sector types:** What sector types exist on the overland and their movement costs.
3. **Map rendering:** How the map is displayed to players.
4. **Landmarks and entrances:** How overland locations connect to regular rooms.
5. **Map resets:** How the overland is populated with mobs/objects.

### Dragonflight
6. **Dragon travel:** How dragon-based fast travel works. Landing sites. Flight paths.
7. **Integration with overland:** How dragonflight interacts with the coordinate system.

### Special Procedures
8. **`spec_*` functions:** Document every special procedure function. What mob types
   use them. How they are assigned.
9. **Execution timing:** When special procedures are called (from `mobile_update()`).

### Weather & Calendar
10. **Weather system:** Weather states, transitions, effects on gameplay.
11. **Calendar:** In-game months, days, seasons. How time progresses.

---

## Output

- **Append** to `ANALYSIS.md` section `## 22. Overland & Special Systems`.
- **Append** to `COMMANDS.md` any overland/weather commands found.

---

## Acceptance Criteria

- [ ] Overland coordinate system documented.
- [ ] Map rendering mechanism documented.
- [ ] Dragonflight system documented.
- [ ] Every special procedure function documented.
- [ ] Weather state machine documented.
