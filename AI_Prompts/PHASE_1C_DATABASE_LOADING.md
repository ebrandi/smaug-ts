# Phase 1C — Database & Area Loading

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase C**
...
| File | Lines | Why |
|------|-------|-----|
| `db.c` | 8,814 | Database loading — area files, mob/obj/room prototypes, boot-time init |

---

## Specific Instructions

This is one of the largest and most critical files. Take your time.

1. **Boot-time loading sequence:** Document the order in which `db.c` loads data at
   startup. What functions are called from `boot_db()` and in what order.

2. **Area file parsing (`.are` format):**
   - Document every section header recognised (`#AREA`, `#MOBILES`, `#OBJECTS`,
     `#ROOMS`, `#RESETS`, `#SHOPS`, `#REPAIRS`, `#SPECIALS`, etc.).
   - For each section, document the exact text format — field order, delimiters,
     string terminators (`~`), numeric formats.
   - **Mobile complexity levels:** Document the exact parsing differences between
     Simple (S), Complex (C), and Very Complex (V) mobile formats.
   - **Object value semantics:** Document how `value[0]` through `value[5]` are
     interpreted differently per `item_type`.

3. **Vnum hash tables:** How `mob_index_hash`, `obj_index_hash`, `room_index_hash`
   are populated. Hash function. Collision handling.

4. **Prototype system:** How `mob_index_data`, `obj_index_data`, `room_index_data`
   serve as templates. How instances are created from prototypes.

5. **String reading utilities:** `fread_string()`, `fread_number()`, `fread_word()`,
   `fread_letter()` — exact parsing behaviour.

6. **Error handling:** What happens when area files contain errors. How `bug()` and
   `log_string()` are used.
...
- **Append** to `ANALYSIS.md` section `## 3. Database & Area Loading`.

---

## Acceptance Criteria

- [ ] Complete `boot_db()` loading sequence documented.
- [ ] Every `.are` section format documented with exact field layouts.
- [ ] Mobile S/C/V format differences documented.
- [ ] Object value semantics per item_type documented.
- [ ] Vnum hash table mechanics documented.
- [ ] String reading utilities documented.
