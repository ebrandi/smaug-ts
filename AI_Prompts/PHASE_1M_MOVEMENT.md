# Phase 1M — Movement & Exits

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase M**
...
| File | Lines | Why |
|------|-------|-----|
| `act_move.c` | ~3,200 | Movement — direction commands, doors, mounts, followers, sector costs |

---

## Specific Instructions

1. **`move_char()` / `do_north()` etc.:** The core movement function. Document the
   complete validation and execution sequence:
   - Exit existence check.
   - Door state check (closed, locked, secret).
   - Movement cost calculation per `sector_type` (exact table).
   - Encumbrance effects on movement cost.
   - `AFF_FLYING` and its effect on sector costs.
   - Room flag checks (`ROOM_NO_MOB`, `ROOM_DEATH`, `ROOM_PRIVATE`, etc.).
2. **Mount system:** How `char_data.mount` affects movement — cost reduction,
   mount stamina, dismount triggers.
3. **Follower cascade:** How leader movement triggers follower movement. Group
   movement mechanics.
4. **Door mechanics:** `do_open()`, `do_close()`, `do_lock()`, `do_unlock()`,
   `do_pick()` — key requirements, skill checks.
5. **Teleportation:** How `room_index_data.tele_vnum` / `tele_delay` work.
6. **Flight:** `AFF_FLYING` — what it enables/disables.
7. **Sector-specific behaviour:** Water sectors requiring boats, underwater
   requiring water breathing, air sectors requiring flight.
8. **Movement messages:** Entry/exit messages, sneaking, hidden movement.

---

## Output

- **Append** to `ANALYSIS.md` section `## 13. Movement & Exits`.
- **Append** to `COMMANDS.md` any movement commands found.

---

## Acceptance Criteria

- [ ] Complete movement validation sequence documented.
- [ ] Sector type → movement cost table extracted.
- [ ] Mount system documented.
- [ ] Follower cascade documented.
- [ ] Door mechanics documented.
- [ ] Teleportation mechanics documented.
