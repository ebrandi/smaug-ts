# Phase 1N — Objects & Inventory

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase N**
...
| File | Lines | Why |
|------|-------|-----|
| `act_obj.c` | 4,617 | Object interaction — get, drop, wear, remove, eat, drink, containers |

---

## Specific Instructions

1. **`do_get()`:** Get from room, get from container, get all, get all from container.
   Weight and capacity checks.
2. **`do_drop()` / `do_put()`:** Drop to room, put in container. Cursed item checks.
3. **`do_wear()` / `do_remove()`:** Wear location resolution, layer system
   (`obj_data.layers`), multi-slot items. How wearing applies affects.
4. **`do_eat()` / `do_drink()`:** Hunger/thirst system. How food/drink items are
   consumed. Poison mechanics.
5. **`do_sacrifice()`:** Object destruction for gold/favour.
6. **`do_give()`:** Object transfer between characters. Weight checks.
7. **Container system:** How nested containers work. Capacity limits. Locked containers.
8. **Object value semantics:** Cross-reference with Phase 1C — how `value[0..5]` are
   used for each `item_type` in the action handlers.
9. **Item layer system:** How `layers` field enables wearing multiple items in the
   same wear location.
10. **Object timers:** How `obj_data.timer` causes object decay.

---

## Output

- **Append** to `ANALYSIS.md` section `## 14. Objects & Inventory`.
- **Append** to `COMMANDS.md` any object commands found.

---

## Acceptance Criteria

- [ ] All object interaction commands documented.
- [ ] Container nesting mechanics documented.
- [ ] Layer system documented.
- [ ] Hunger/thirst/poison mechanics documented.
- [ ] Object timer/decay documented.
