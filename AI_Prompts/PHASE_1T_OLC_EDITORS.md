# Phase 1T — OLC (Online Creation) Editors

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase T**
...
| File | Lines | Why |
|------|-------|-----|
| `build.c` | 11,286 | Main OLC — room/mob/obj/area editing |
| `ibuild.c` | 3,976 | Interior building editor |
| `omedit.c` | ~500 | Object editor variant |
| `oredit.c` | ~500 | Object editor variant |
| `ooedit.c` | ~500 | Object editor variant |

---

## Specific Instructions

**Note:** `build.c` is the second-largest file (11,286 lines). Focus on architecture
and command patterns rather than documenting every single sub-command.

### `build.c`
1. **OLC architecture:** How the online building system is structured. The `olc` pointer
   in `descriptor_data`. How players enter/exit edit modes using `SUB_*` substates.
2. **Room editor (`redit`):** What room properties can be edited. Command list.
3. **Mob editor (`medit`):** What mob properties can be edited. Command list.
4. **Object editor (`oedit`):** What object properties can be edited. Command list.
5. **Area editor (`aedit`):** Area-level properties. Vnum range management.
6. **Reset editor:** How resets are configured through OLC.
7. **Vnum range enforcement:** How `pc_data.r_range_lo/hi`, `m_range_lo/hi`,
   `o_range_lo/hi` limit builder access.
8. **Area saving from OLC:** How edited data is persisted to `.are` files.
9. **Validation:** What validation is performed on builder input.

### `ibuild.c`
10. **Interior building:** What this adds beyond `build.c`. Interior-specific features.

### Object editor variants
11. **`omedit.c` / `oredit.c` / `ooedit.c`:** How these differ from the object editor
    in `build.c`. Why multiple editors exist (likely iterative development).

---

## Output

- **Append** to `ANALYSIS.md` section `## 20. OLC Editors`.
- **Append** to `COMMANDS.md` section `## Builder Commands` — list all OLC commands
  with their trust requirements.

---

## Acceptance Criteria

- [ ] OLC architecture documented (edit modes, substates, descriptor linkage).
- [ ] Room/mob/obj/area editor command lists documented.
- [ ] Vnum range enforcement documented.
- [ ] Area save mechanism documented.
- [ ] All OLC commands added to `COMMANDS.md`.
