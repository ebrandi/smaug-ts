# Phase 1D — Persistence & Resets

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase D**
...
| File | Lines | Why |
|------|-------|-----|
| `save.c` | 3,500 | Player save/load — `.plr` file format, character persistence |
| `reset.c` | ~800 | Area reset execution — repopulating rooms with mobs and objects |
...
### `save.c`
1. **Player file format (`.plr`):** Document the exact key-value text format.
   Every field written by `fwrite_char()` and read by `fread_char()`.
2. **Password handling:** How passwords are stored (SHA256, unsalted — note security
   implications for the port).
3. **Affect persistence:** How `affect_data` is saved and restored.
4. **Equipment persistence:** How worn/carried items are saved.
5. **Save triggers:** What events cause a player save (death, quit, auto-save, hotboot).
6. **Corpse persistence:** How player corpses survive reboots.

### `reset.c`
7. **Reset commands:** Document each reset type (`M`, `O`, `P`, `G`, `E`, `D`, `R`)
   with exact semantics — what each creates/places and the conditions checked.
8. **Reset execution order:** How resets are processed within an area.
9. **Instance limits:** How `reset_data.arg3` (max count) is enforced.
10. **Nested resets:** How `P` (put in object), `G` (give to mob), `E` (equip on mob)
    relate to the preceding `M` or `O` reset.

---

## Output

- **Append** to `ANALYSIS.md` section `## 4. Persistence & Resets`.

---

## Acceptance Criteria

- [ ] Complete `.plr` file format documented.
- [ ] Password storage mechanism documented with security notes.
- [ ] All reset types documented with exact semantics.
- [ ] Reset execution order and nesting documented.
