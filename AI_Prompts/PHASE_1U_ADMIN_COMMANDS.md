# Phase 1U — Admin & Immortal Commands

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase U**
...
| File | Lines | Why |
|------|-------|-----|
| `act_wiz.c` | 13,078 | Wizard/admin commands — the largest file in the codebase |
| `ban.c` | ~400 | IP/character banning |
| `imm_host.c` | ~300 | Immortal host filtering |
| `adminlist.c` | ~200 | Admin list management |

---

## Specific Instructions

**Note:** `act_wiz.c` is 13,078 lines. Use a systematic approach:
- First, scan for all `do_*` function definitions to build a complete command list.
- Then, for each command, document: name, trust level, purpose, key parameters.
- For the **20 most important** commands (goto, transfer, slay, purge, restore, peace,
  bamfin/bamfout, wizinvis, holylight, mset, oset, rset, sset, mfind, ofind, rfind,
  shutdown, reboot, hotboot, copyover), provide detailed analysis.
- For the remaining commands, a one-line description is sufficient.

### `act_wiz.c`
1. **Complete command inventory:** Every `do_*` function with trust level and purpose.
2. **Top 20 detailed analysis:** Full algorithm documentation for the most important
   admin commands.
3. **`mset` / `oset` / `rset`:** What properties can be modified on mobs, objects,
   and rooms. This is critical for understanding the full data model.
4. **`sset`:** How skills/spells are configured at runtime.
5. **Shutdown/reboot/hotboot:** The complete shutdown sequence. How hotboot differs
   from cold reboot.

### `ban.c`
6. **Ban system:** IP bans, character bans, site bans. How bans are stored and checked.

### `imm_host.c`
7. **Host filtering:** How immortal logins are restricted by hostname/IP.

### `adminlist.c`
8. **Admin list:** How the admin roster is maintained.

---

## Output

- **Append** to `ANALYSIS.md` section `## 21. Admin & Immortal Commands`.
- **Append** to `COMMANDS.md` section `## Immortal Commands` — complete list with
  trust levels.
...
- [ ] Every `do_*` function in `act_wiz.c` listed with trust level and purpose.
- [ ] Top 20 commands documented in detail.
- [ ] `mset`/`oset`/`rset` property lists documented.
- [ ] Shutdown/reboot/hotboot sequence documented.
- [ ] Ban system documented.
- [ ] All immortal commands added to `COMMANDS.md`.
