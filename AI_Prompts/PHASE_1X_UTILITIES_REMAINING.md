# Phase 1X — Utilities & Remaining Files

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase X**
...
| File | Lines | Why |
|------|-------|-----|
| `dice.c` | ~200 | Dice rolling utilities |
| `misc.c` | ~500 | Miscellaneous utilities |
| `track.c` | ~400 | BFS pathfinding |
| `sha256.c` | ~500 | SHA256 hashing (third-party) |
| `starmap.c` | ~300 | Star map utility |
| `renumber.c` | ~300 | Room renumbering utility |
| `mapout.c` | ~300 | Map export utility |
| `grub.c` | ~200 | Database cleanup utility |
| `ident.c` | ~200 | Identification system |

---

## Specific Instructions

1. **`dice.c`:** Dice rolling function(s). Format: `NdS+M`. How randomness is seeded.
2. **`track.c`:** BFS pathfinding algorithm. How `do_track()` finds a path to a target.
   What data structures are used for the BFS queue. Maximum search depth.
3. **`misc.c`:** Catalogue all utility functions. Note which are used widely across
   the codebase.
4. **`sha256.c`:** Confirm this is used for password hashing. Note if it is a standard
   implementation or custom.
5. **Remaining files:** For each remaining file, provide a brief overview (2–3 sentences)
   of what it does and whether it contains anything significant for the port.
6. **Catch-all:** Check `STRUCTURE.md` for any `.c` files not covered by sub-phases
   1A–1W or this sub-phase. Document any gaps.

---

## Output

- **Append** to `ANALYSIS.md` section `## 24. Utilities & Remaining Files`.

---

## Acceptance Criteria

- [ ] Dice rolling mechanics documented.
- [ ] BFS pathfinding algorithm documented.
- [ ] SHA256 usage confirmed.
- [ ] All remaining `.c` files accounted for.
- [ ] No `.c` files from `STRUCTURE.md` left undocumented.
