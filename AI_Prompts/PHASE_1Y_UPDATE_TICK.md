# Phase 1Y — Update & Tick System

> **SMAUG 2.0 Legacy Codebase Analysis — Phase 1 Sub-Phase Y**
...
| File | Lines | Why |
|------|-------|-----|
| `update.c` | 4,067 | Tick-driven updates — violence, mobile AI, weather, time, area resets, affects |

---

## Specific Instructions

This file is the heartbeat of the MUD. Every periodic game event flows through here.

1. **`violence_update()`:** Called on `PULSE_VIOLENCE`. What happens each combat tick
   for every fighting character. Cross-reference with Phase 1I (fight.c).
2. **`mobile_update()`:** Called on `PULSE_MOBILE` (~4s). Document the complete NPC AI
   loop:
   - Special procedure execution.
   - MUDprog trigger evaluation.
   - Wandering logic (random movement).
   - Flee behaviour (when to flee, direction selection).
   - Scavenging (picking up objects).
   - Aggressive mob targeting (who they attack and why).
   - Time-based triggers.
3. **`char_update()` / tick update:** Called on `PULSE_TICK` (~70s):
   - HP/mana/move regeneration formulas (exact — position-dependent, hunger-dependent).
   - Hunger/thirst decrement.
   - Affect duration countdown and expiration.
   - Poison damage.
   - `char_data.timer` decrement and idle handling.
4. **`area_update()`:** Called on `PULSE_AREA`. How area age is tracked and when
   resets are triggered.
5. **`aggr_update()`:** How aggressive mobs select targets.
6. **`obj_update()`:** Object timer countdown, object decay.
7. **`weather_update()` / `time_update()`:** Weather and time progression (may also
   be in weather.c/timezone.c — note cross-references).
8. **XP calculation:** If XP award formulas are in this file, extract them exactly.
   Level-difference scaling, group XP splitting, XP loss on death.

---

## Output

- **Append** to `ANALYSIS.md` section `## 25. Update & Tick System`.

---

## Acceptance Criteria

- [ ] Every update function documented with its pulse frequency.
- [ ] HP/mana/move regeneration formulas extracted exactly.
- [ ] Mobile AI loop fully documented.
- [ ] Area reset triggering logic documented.
- [ ] Affect expiration mechanics documented.
- [ ] XP formulas extracted (if present in this file).
